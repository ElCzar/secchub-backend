package co.edu.puj.secchub_backend.planning.exception;

/**
 * Exception thrown when a teaching assistant is not found in the planning module.
 */
public class TeachingAssistantNotFoundException extends RuntimeException {
    
    public TeachingAssistantNotFoundException(String message) {
        super(message);
    }
    
    public TeachingAssistantNotFoundException(String message, Throwable cause) {
        super(message, cause);
    }
}